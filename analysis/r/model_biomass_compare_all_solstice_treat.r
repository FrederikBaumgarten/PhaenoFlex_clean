...
#extract control
summary_effects_df_sub[summary_effects_df_sub$Combination == "control_Prvi", "mean"]
...
#unique(summary_effects_df$treatment)
# Convert `treatment` and `species` to factors to maintain ordering in the plot
summary_effects_df_sub$treatment <- factor(summary_effects_df_sub$treatment, levels = c("control", "drought2", "defol2", "controlheat", "GSextend"))
...
# Define custom labels for the facets
custom_labels <- c(Acma = "Acer macrophyllum", Bepa = "Betula papyrifera", Pico = "Pinus contorta", Potr = "Populus trichocarpa", Prvi = "Prunus virginiana", Quma = "Quercus garryana", Sese = "Sequoia sempervirens")
position_dodge_width <- 0.5 # Adjust position_dodge to cluster species within a treatment
...
biomass_post_treat_sub <- ggplot(summary_effects_df_sub, aes(y = treatment, x = mean_sub, color = species)) +
  ...
  geom_errorbar(aes(xmin = mean_sub - sd, xmax = mean_sub + sd), width = 0.2, position = position_dodge(width = position_dodge_width)) +
  ...
  scale_y_discrete(labels = function(y) {
    ifelse(y == "defol2", "Defoliation",
    ifelse(y == "drought2", "Drought",
    ifelse(y == "controlheat", "Heat",
    ifelse(y == "GSextend", "GS_extend",
    ifelse(y == "def1", "Def_1",
    ifelse(y == "def2", "Def_2",
...
# Export the plot as a PDF
ggsave(filename = "/Users/frederik/github/PhaenoFlex_clean/analysis/output/posteriors/biomass_post_solst_treat_sub.pdf", plot = biomass_post_treat_sub, width = 4, height = 5)
