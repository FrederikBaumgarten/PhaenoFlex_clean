...
schedule_acma <- subset(schedule, id =="Acma")
schedule_bepa <- subset(schedule, id =="Bepa")
schedule_pico <- subset(schedule, id =="Pico")
schedule_quma <- subset(schedule, id =="Quma")
schedule_sese <- subset(schedule, id =="Sese")
...
cc1_stan <- cchamber1
cc1_stan$datetime <- strptime(cc1_stan$Date.time.pdt, format = "%m-%d-%Y %H:%M:%S")
head(cc1_stan)
...
dro_2_forplot$julian_decim <- dro_2_forplot$julian_decim+37
# Idem for drought 3
dro_3_forplot <- dro_1_forplot[c(1:28),]
dro_3_forplot$julian_decim <- dro_3_forplot$julian_decim+60
# Idem for drought 4
dro_4_forplot <- dro_3_forplot[c(1:28),]
dro_4_forplot$julian_decim <- dro_4_forplot$julian_decim+30
...
merged_temp_daylength <- merge(daily_totem_2022, daylength_df[, c("daylength", "julian")], by = "julian", all = TRUE)
...
tempgraph <- ggplot(data = merged_temp_daylength, aes(x = julian, y = meantemp)) +
  ...
  scale_y_continuous(
    ...
    sec.axis = sec_axis(~./1.9, name = "Daylength (hours)", 
                        breaks = seq(0, max(merged_temp_daylength$daylength * 1.9), by = 5))
  ) +
  # Add Dormancy 
  ...
  geom_text(x=schedule_prvi$dormancy_start,
            ...
            hjust = -0.51, #adjust the position of text on the x axis
  ...
  geom_segment(x=schedule_prvi$dormancy_end, 
               ...
               yend=(daily_totem_2022$meantemp[daily_totem_2022$julian == schedule_prvi$dormancy_end])+1, colour="black", #go extract the value of defol 1 start
               arrow = arrow(length = unit(0.02, "npc"))) +
  geom_text(x = schedule_prvi$dormancy_end, 
            y = daily_totem_2022$meantemp[daily_totem_2022$julian == schedule_prvi$dormancy_end] - 1,
            label = "Transfer to ambiant conditions",
            hjust = -0.35, # adjust the text position on the y axis
            vjust = -0.5,
            angle = 90,
            size = 3) + # adjust the text position on the x axis
  # Add arrow for the harvest date --> NEEDS TO BE CHANGED FOR THE REAL DATE
  geom_segment(x=schedule_prvi$harvest_date, 
               y=30, 
               xend=schedule_prvi$harvest_date, 
               yend=(daily_totem_2022$meantemp[daily_totem_2022$julian == schedule_prvi$harvest_date]) + 2, colour="black", #go extract the value of harvest date
               arrow = arrow(length = unit(0.02, "npc"))) +
  geom_text(x = schedule_prvi$harvest_date, 
            y = daily_totem_2022$meantemp[daily_totem_2022$julian == schedule_prvi$harvest_date] - 1,
            label = "Harvest",
            hjust = -2, # adjust the text position on the y axis
            vjust = -0.5,
            angle = 90,
            size = 3) + # adjust the text position on the x axis
  # Add Drought 1
  ...
  geom_segment(x=schedule_prvi$defol_1_start, 
               ...
               yend=(daily_totem_2022$meantemp[daily_totem_2022$julian == schedule_prvi$defol_1_start])-1, aes(colour="Defoliation 1"), #go extract the value of defol 1 start
               arrow = arrow(length = unit(0.02, "npc"))) +
  # Add Drought 2
  ...
  geom_segment(x=schedule_prvi$defol_2_start, 
               ...
               yend=(daily_totem_2022$meantemp[daily_totem_2022$julian == schedule_prvi$defol_2_start])-0.5, aes(colour="Defoliation 2"), #go extract the value of defol 2 start
               arrow = arrow(length = unit(0.02, "npc")))+
  # Add Drought 3
  geom_line(data=dro_3_forplot, aes(julian_decim, temp, colour="Drought 3"), size=0.5) +
  # Add Drought 4
  geom_line(data=dro_4_forplot, aes(julian_decim, temp, colour="Drought 4"), size=0.5) +
  #Set titles
  ggtitle("") +
  scale_color_manual(name = "Treatments", values = c("Drought 1" = "gold1",
                                                 "Drought 2" = "gold2",
                                                 "Drought 3" = "gold3",
                                                 "Drought 4" = "gold4",
                                                 "Defoliation 1" = "limegreen",
                                                 "Defoliation 2" = "limegreen",
                                                 "Heat Wave" = "darkorange2")) +
...
ggsave("output/tempgraph.pdf", tempgraph, dpi = 300, width = 7, height = 5)
